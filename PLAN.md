...
7. **둘이 같이 결정하게 만들기**: 한 사람이 바닥부터 조사·비교·계약을 떠안지 않도록, 함께 정해야 할 시점과 판단 재료를 한 화면에 모은다. 역할 분담은 보조 기능으로 낮추고, 미루면 손해 보는 혜택·추가금·마감 신호와 "오늘은 이것만 고르면 된다"는 결정 패킷을 제품 흐름에 녹인다. 상세 리서치 메모는 `PRODUCT_INSIGHTS.md`, 구현 계획은 `DECISION_MAP_PLAN.md` 참고.
